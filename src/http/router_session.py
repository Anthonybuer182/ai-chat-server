...
@router.post("/create")
async def create(request:SessionListRequest,user: UserDB = Depends(get_user),db: AsyncSession = Depends(get_db)):
    session = await create_session(db=db,session=request)
...
async def delete(id:str,user: UserDB = Depends(get_user),db: AsyncSession = Depends(get_db)):
    result = await delete_session(db=db,session_id=id)
    return success_response(data=result)

@router.post("/edit")
async def edit(request:SessionListRequest,user: UserDB = Depends(get_user),db: AsyncSession = Depends(get_db)):
    session = await edit_session(db=db,session=request)
    return success_response(data=jsonable_encoder(session))
